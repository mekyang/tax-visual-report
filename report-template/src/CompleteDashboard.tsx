...
export default function CompleteDashboard() {
  ...
  const drillDownData = useMemo(() => {
    ...
    if (!selectedDistrict) {
      ...
      filteredRows.forEach((row: any) => {
        ...
        paths.forEach((path: string) => {
          const nodes = path.split('->').map((n: string) => n.trim()).filter(Boolean);
          let cur = tree;
    ...
    return Object.entries(current)
      .filter(([name]: any) => !excludedCats.some(excluded => name.includes(excluded)))
      ...
      <div className="max-w-[1200px] mx-auto px-6 mt-4">
        ...
        <GlassCard icon={Database}>
          <div className="mb-6 text-slate-600 leading-relaxed text-base bg-slate-50 p-5 rounded-xl border border-slate-100 indent-8">
            {formatDate(meta.dateMin)}至{formatDate(meta.dateMax)}，全量监测周期内山东税务税费服务热线累计来电
            <span className="text-sky-600 font-bold text-xl mx-1">{meta.totalRecords.toLocaleString()}</span>通。来电情况如下：
          </div>
        </GlassCard>
        ...
        <GlassCard icon={PhoneCall}>
          ...
          >
            {(() => {
              ...
              return (
                <span>
                  {formatDate(meta.dateMin)}至{formatDate(meta.dateMax)}，问题解答类咨询共计
          ...
          <div className="mb-8 p-5 bg-gradient-to-r from-slate-50 to-sky-50 rounded-xl border border-slate-200">
            ...
            <div className="grid grid-cols-3 gap-4">
              {globalTopicSummary.slice(0, 3).map((ch: any, idx: number) => {
                const top3Sum = ch.hotTopics.slice(0, 3).reduce((s: number, t: any) => s + t.value, 0);
                const concentration = ch.total > 0 ? Math.min((top3Sum / ch.total) * 100, 100).toFixed(1) : '0.0';
                return (
                  <div key={idx} className="bg-white p-5 rounded-xl border border-slate-100 shadow-sm flex flex-col">
                    ...
                    <div className="flex-1 flex flex-col justify-center">
                      <div className="text-sm text-slate-600 mb-2">该大类下，排名前三的细分问题合计占比：</div>
                      ...
                      <div className="mt-auto text-xs text-slate-500 bg-slate-50 p-3 rounded-lg leading-relaxed border border-slate-100">
                        <span className="font-bold text-slate-700">包含细分：</span>
                        {ch.top3.join('、')}
                      </div>
                    </div>
                  </div>
        ...
        {/* ─────────── 三、服务投诉渠道来电情况 ─────────── */}
        <SectionHeader title="三、投诉来电情况" step="03" />
          ...
          {reportData.complaintTotal > 0 ? (
            <>
              <div className="mb-6 text-slate-600 leading-8 text-base bg-rose-50/60 p-5 rounded-xl border border-rose-100 indent-8">
                {formatDate(meta.dateMin)}至{formatDate(meta.dateMax)}，全省共实际受理服务投诉
          ...
          {taxComplaintTotal > 0 ? (
            <>
              <div className="mb-6 text-slate-600 leading-8 text-base bg-amber-50/60 p-5 rounded-xl border border-amber-100 indent-8">
                {formatDate(meta.dateMin)}至{formatDate(meta.dateMax)}，全省共受理涉税投诉举报
